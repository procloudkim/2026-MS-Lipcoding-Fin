...
check("matchCases 카시트", matchCases("아기 카시트 추천").some((c) => c.id === "carseat"));
check("matchCases 유모차", matchCases("유모차 안전한 제품").some((c) => c.id === "stroller"));
check("matchCases 물티슈", matchCases("아기 물티슈 성분").some((c) => c.id === "wipes"));
check("matchCases 베이비파우더", matchCases("베이비파우더 써도 되나요").some((c) => c.id === "powder"));
check("matchCases 속싸개->수면", matchCases("신생아 속싸개 수면").some((c) => c.id === "sleep"));
...
check("normalizeBrief 출처 이름->공식 URL 부착", brief.sources[0].url === "https://www.mfds.go.kr");
...
check("fallbackBrief 출처 URL 존재", fb.sources.every((s) => /^https?:\/\//.test(s.url)));
...
// 확장된 시드: 비-BPA 질의도 구체적 결과
const fbCar = fallbackBrief("아기 카시트 추천");
check("fallbackBrief 카시트 카테고리 이동/외출", fbCar.category === "이동/외출");
check("fallbackBrief 카시트 물리축 포함", fbCar.evidence.some((e) => e.axis === "physical"));
check("fallbackBrief 카시트 KC 체크리스트", fbCar.checklist.some((c) => c.includes("KC") || c.includes("R129")));

const fbWipes = fallbackBrief("아기 물티슈 성분 안전한지");
check("fallbackBrief 물티슈 화학축 포함", fbWipes.evidence.some((e) => e.axis === "chemical"));
check("fallbackBrief 물티슈 출처 식약처", fbWipes.sources.some((s) => s.name.includes("식약처")));

const fbPowder = fallbackBrief("베이비파우더 써도 되나요");
check("fallbackBrief 베이비파우더 사례매칭->CAUTION", fbPowder.verdict === "CAUTION");
check("fallbackBrief 베이비파우더 대안 존재", fbPowder.alternatives.length >= 1);
